...
async function createCalendarEvent({ title, description, startIso, endIso, attendees }) {
  ...
  try {
    ...
    console.error('Calendar creation failed, falling back to dummy link:', e);
    return {
      htmlLink: 'https://calendar.google.com',
      hangoutLink: `https://meet.google.com/${Math.random().toString(36).slice(2, 10)}`,
      id: Math.random().toString(36).slice(2),
...
app.post('/api/book', async (req, res) => {
  try {
    ...
    // Accept partial input; fall back to now + duration
    const now = new Date();
    const start = (date && time) ? new Date(`${date}T${time}:00Z`) : now;
    const startIso = isNaN(start.getTime()) ? now.toISOString() : start.toISOString();
    const endFrom = new Date(startIso);
    const endIso = new Date(endFrom.getTime() + durationMinutes * 60000).toISOString();
    ...
    const event = await createCalendarEvent({
      ...
      startIso,
      endIso,
      attendees: [userEmail, process.env.COUNSELOR_EMAIL || 'counselor@example.com'].filter(Boolean),
    });
    ...
    if (transporter && emails.length > 0) {
      try {
        ...
        console.error('Email sending failed, continuing without email:', e);
      }
    }
